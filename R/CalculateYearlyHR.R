...
##
##      GOAL OF THE FUNCTION: Calculate yearly population home range
##
##
##      CREDIT: Gaëlle Picon (picon.gaelle@gmail.com)
#_______________________________________________________________________________________________________________

calculate_yearly_hr <- function(
  region,
  species,
  input_dir = NULL,
  output_dir = NULL,
  ...
  # Count how many birds are left per year
  birds_per_year <- data_unique %>%
    mutate(year = lubridate::year(as.Date(t_original))) %>%
    group_by(year) %>%
    summarise(n_birds = n_distinct(bird_id), .groups = "drop")

  # Export the summary
  write.csv(
    birds_per_year,
    file.path(output_dir, "birds_per_year.csv"),
    row.names = FALSE
  )
  ...
  # Extract all years
  years <- sort(unique(lubridate::year(as.Date(data_unique$t_original))))
  message(sprintf("Years found: %s", paste(years, collapse = ", ")))

  # Process each year separately
  for (yr in years) {
    message(sprintf("Calculating home range for year %d", yr))

    # Create subdirectory for the processed year if it does not exist yet
    yr_dir <- file.path(output_dir, yr)
    if (!dir.exists(yr_dir)) {
      dir.create(yr_dir, recursive = TRUE)
    }

    # Filter data for this year
    data_yr <- data_unique %>%
      filter(lubridate::year(as.Date(t_original)) == yr)

    # Calculate population home range
    message("Calculating population home range")

    # Convert the time column
    data_yr$t_original <- lubridate::ymd_hms(data_yr$t_original)
    ...
    sf::st_write(
      ...
      file.path(
        yr_dir,
        paste0("GF_Causses_HR95_href_", method, "_", yr, ".shp")
      ),
      delete_layer = TRUE,
      quiet = TRUE
    )
    ...
    sf::st_write(
      ...
      file.path(
        yr_dir,
        paste0("GF_Causses_HR90_href_", method, "_", yr, ".shp")
      ),
      delete_layer = TRUE,
      quiet = TRUE
    )
    ...
    sf::st_write(
      ...
      file.path(
        yr_dir,
        paste0("GF_Causses_HR50_href_", method, "_", yr, ".shp")
      ),
      delete_layer = TRUE,
      quiet = TRUE
    )
